...
This model attempts to run algorithms with nonlinear model terms, with interaction effects, squared terms and other transformations, such as step functions. Multivariate Adaptive Regression Splines (MARS) will be employed to explore nonlinear model terms with automatic functions, by using the **earth** package.
...
The summary of the MARS model indicates a high value for its R², which is a good performance for the model, and 15 of the predictors have been kept. The list of coefficients also show the knots for the step functions.
...
Hyperparameters minimize prediction error by choosing how many terms in the final model and the complexity of the interactions. Grid search provides a set of different combinations of these two combinations. This is intended to prevent overfitting (the model does well for training data, but not well for testing or validating data) and underfitting (where the model is too simple to capture relevant patterns.)
