...
The research question of my MT consists of assessing the fiscal impact (in terms of tax revenue) of a greater adoption of electrical vehicles (EV) in Brazil, focusing exclusively on the shift from passenger cars with interla combustion engines (ICE) to plug-in/battery powered electric vehicles (BEV). For that, I intend to calculate projections based on three estimated variables:

1.  **Vehicle Stock:** total registered BEVs in a given moment (input) and revenues from registration and annual vehicle tax (output).

2.  **Energy Use:** vehicle stock and vehicle activity (input) and revenues from taxes on fuel (output).

To obtain the input for the first variable, I intend to use historical data on the country's fleet of passenger vehicles of all kinds and other relevant data (population size per state and city, income distribution on state level, vehicle prices) to estimate the current rates of BEV adoption in different states, and then use this information to extrapolate the data or run projections on how the BEV stock might grow in a time frame of 10-15 years.

To obtain the input for the second variable, I intend to use data on fuel sales and fuel consumption, as well as other information (such as average vehicle activity, measured in kilometers driven per year in different states).
...
While useful to show the distribution of BEVs in the country, the states with a darker shade of blue above are also the states with the larger population sizes in Brazil. While this correlation is somewhat obvious, it is useful to understand where the absolute number of BEVs is most concentrated.
...
When looking into BEV per capita, States with a darker shade of blue (showing a bigger share of BEV per capita) may point to states with a greater level of urbanization (as PR and its capital, Curitiba, or the country's capital, Brasilia), or to specific local policies.
...
Exploring the data visually, I find that population has a clear positive correlation with the BEV fleet size, though this may be somewhat obvious. I am not yet entirely sure if I should move forward with this variable as a predictor.
...
When plotting the data, BEV prices and BEV stock seem to have some degree of correlation, suggesting that more BEVs are bought as more affordable vehicles are available in the Brazilian market. However, the data points are oddly spread in the plot, as I do not have an explanation for these two clusters.

I used data on the cheapest BEV model ("min_price_electric") available in the Brazilian market in a given month, and the BEV stock size in the corresponding month.
...
The summary of the MARS model indicates a high value for its R², which is a good performance for the model. Also, the 15 of the predictors have been kept, with 30 terms selected for the best performing model.
...
A second attempt at the MARS algorithm includes interactions. The variables describing the model performance have been included below, as well as a plot showing the relevance of different variables:
...
Based on the coefficients, statistical significance and R² values, the linear models obtained (with and without fixed effects) have shown a good performance. The corresponding tables suggest that grown in the BEV stock can be explained by population size, income levels, BEV prices. Additionally, Regularized Regressions and Multivariate Adaptive Regressions have been attempted, but the algorithms generated have not performed feature selection in a relevant way.

The next step now will be to calculate expected values of BEV stock for the coming years. For that, I will attempt two approaches:
